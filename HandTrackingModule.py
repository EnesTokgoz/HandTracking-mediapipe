...
class handDetector():
    def __init__(self, mode = False, maxHands = 2, detectionCon = 0.5, trackCon = 0.5):
        ...
        self.tipIds = [4, 8, 12, 16, 20]#parmak uçları
    ...
    def findPosition(self, img, handNo = 0, draw = True ):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(myHand.landmark):  # eldeki noktaların koordinatları
                # print(id,lm)
                h, w, c = img.shape  # görüntünün değerlerini sabite eşitleme
                cx, cy = int(lm.x * w), int(lm.y * h)  # ondalık koordinatları piksele çevirme
                xList.append(cx)
                yList.append(cy)
                #print(id, cx, cy)  # piksel koorrdinatları yazdırma
                self.lmList.append([id, cx, cy])
                #if id == 4:  # seçilen parmağa daire çizme
                if draw:
